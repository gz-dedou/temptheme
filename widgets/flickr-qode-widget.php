...
class Qode_Flickr_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$this->WP_Widget('qode-flickr-widget', 'Qode Flickr Widget', array('classname' => 'flickr', 'description' => 'Qode Flickr Widget for user photo stream!'), array('id_base' => 'qode-flickr-widget'));
			...
			if($userid->user->id!='') {
				...
				<ul class='flickr_widget'>
					...
					<li class='flickr_single_photo'>
						...
						<a href='<?php echo $url_item->user->url; ?><?php echo $photo['id']; ?>' target='_blank' title="<?php echo $photo['title']; ?>">

							<img src='<?php $url = "http://farm" . $photo['farm'] . ".static.flickr.com/" . $photo['server'] . "/" . $photo['id'] . "_" . $photo['secret'] . '_s' . ".jpg"; echo $url; ?>' alt='<?php echo $photo['title']; ?>' width="<?php echo $img_widht; ?>" height="<?php echo $img_height; ?>" />
