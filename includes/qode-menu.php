...
if (!class_exists('qode_type1_walker_nav_menu')) {
	...
	// add classes to ul sub-menus
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && ($qode_options_subway['page_transitions'] == "0")):
			...
			if($item->hide == ""){
				...
				$item_output .= '<span></span>'.apply_filters( 'the_title', $item->title, $item->ID );
				$item_output .= '</a>';
			}
		 ...
		 function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
				...
				if($item->nolink == ""){
					...
					$item_output .= '<span class="mobile_arrow"></span></h3>';
				}
			}
			$item_output .= $args->after;
